dashboard "aws_s3_bucket_public_access_report" {
  ...
  container {
    ...
    card {
      query = aws_insights.query.s3_bucket_count
      width = 2
    }

    card {
      query = aws_insights.query.s3_bucket_public_policy_count
      width = 2
    }

    card {
      query = aws_insights.query.s3_bucket_block_public_acls_disabled_count
      width = 2
    }

    card {
      query = aws_insights.query.s3_bucket_block_public_policy_disabled_count
      width = 2
    }

    card {
      query = aws_insights.query.s3_bucket_ignore_public_acls_disabled_count
      width = 2
    }

    card {
      query = aws_insights.query.s3_bucket_restrict_public_buckets_disabled_count
      width = 2
    }
  ...
  table {
    ...
    column "Name" {
      href = "${aws_insights.dashboard.s3_bucket_detail.url_path}?input.bucket_arn={{.ARN | @uri}}"
    }

    query = aws_insights.query.s3_bucket_public_access_table
  }

}


# If you chose to insert direct queries vs leverage from Insights Mod
